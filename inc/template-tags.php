...
// Comments link.
if ( ! function_exists( 'architect_comment_link' ) ) :
	/**
	 * Prints HTML with the comment link for the current post.
	 */
	function architect_comment_link() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			echo architect_get_icon_svg( 'comment', 16 );

			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'architect' ), get_the_title() ) );
...
if ( ! function_exists( 'architect_entry_footer' ) ) :
	...
	function architect_entry_footer() {
		...
		edit_post_link(
			...
			'<span class="edit-link">' . architect_get_icon_svg( 'edit', 16 ),
			'</span>'
		);
	}
endif;
